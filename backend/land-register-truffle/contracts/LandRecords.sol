...
contract LandRecords {
    ...
    mapping(uint => landTrasferRequest) public TransferRequest;
    ...
    function addLandInspector(address _address,string memory _name, uint _age,string memory _city) public returns(bool){
        ...
        totalInspectors++;
        addressToInspector[_address] = landInspector(totalInspectors,_address,_name, _age,_city);
        return true;
    }

    function returnAllLandIncpectors() public view returns(address[] memory)
    {
    ...
    function registerNewUser(string memory _name,string memory _email ,uint _age, string memory _city,string memory _aadharNumber, string memory _panNumber, string memory _document) public {
        ...
        addressToUser[msg.sender] = user( _name, _email,msg.sender, _age, _city, _document,_aadharNumber,_panNumber,false);
    ...
    function addLand(uint _area, string memory _address,string memory _surveyNum, string memory _document) public {
        require(isUserVerified(msg.sender));
        totalLandsCount++;
        lands[totalLandsCount] = Land(totalLandsCount,msg.sender ,_area, _address, _surveyNum , _document,false,false);
    ...
    function requestforBuy(uint _landId) public
    {
        ...
        totalRequestCount++;
        BuyRequest[totalRequestCount]=landBuyRequest(totalRequestCount,_landId,msg.sender,lands[_landId].landOwner,reqStatus.pending);
    ...
    function acceptRequest(uint _requestId) public
    {
        ...
        transferCount++;
        TransferRequest[transferCount] = landTrasferRequest(transferCount,BuyRequest[_requestId].landId,msg.sender,BuyRequest[_requestId].buyerId,reqStatus.pending);
    ...
    function buyrequesteStatus(uint id) public view returns(bool)
    {
        return (BuyRequest[id].requestStatus == reqStatus.accepted);
    }

    function transferrequesteStatus(uint id) public view returns(bool)
    {
    ...
    function transferOwnership(uint _requestId,string memory documentUrl) public returns(bool)
    {
        ...
        usersLands[TransferRequest[_requestId].buyerId].push(TransferRequest[_requestId].landId);

        uint len=usersLands[TransferRequest[_requestId].sellerId].length;
        for(uint i=0;i<len;i++)
        {
            if(usersLands[TransferRequest[_requestId].sellerId][i]==TransferRequest[_requestId].landId)
            {
                usersLands[TransferRequest[_requestId].sellerId][i]=usersLands[TransferRequest[_requestId].sellerId][len-1];
                //usersLands[TransferRequest[_requestId].sellerId].length--;
        ...
        lands[TransferRequest[_requestId].landId].landOwner=TransferRequest[_requestId].buyerId;
        return true;
    }
