...
const columns = [
  { id: 'reqId', label: 'Request Id', maxWidth: 90 },
  { id: 'landId', label: 'Land Id', maxWidth: 80 },
  { id: 'buyerAddress', label: 'Buyer Address', maxWidth: 80,align: 'center' },
  { id: 'sellerAddress', label: 'Seller Address', maxWidth: 80,align: 'center' },
  {
    id: 'reqStatus',
...
function ActionButton(props) {
  ...
  if (reqState === 'Pending') {
    return(
      ...
      <ButtonGroup variant="contained" aria-label="outlined button group">
        <Button color="success" onClick={async()=>{
          await contract.methods.transferOwnership(reqId,'txDoc').send({from:account,gas:3000000});
    ...
    return(
      <TableCell key={props.column.id} align={props.column.align}>
      <Button disabled color="success">Transfered</Button>
    </TableCell>
    )    
  ...
  useEffect(() => {
    ...
    const fillRows = async()=>{
      ...
      async function fetchData(rId) {
        let a = await contract.methods.TransferRequest(rId).call();
        let reqs='';
      ...
      const loadData = async()=>{
        const data = await contract.methods.allTransferRequestList().call({from:account,gas:3000000});
        accounts = data;
      }
