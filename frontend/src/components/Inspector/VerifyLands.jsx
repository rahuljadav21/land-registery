    ...
    return(
      ...
      <Button onClick={
        async () => {
          ...
          await contract.methods.verifyLand(props.lId.toString()).send({from:account,gas:3000000});
          setverifi('Yes');
        }
                  ...
                  <TableRow hover role="checkbox" tabIndex={-1} key={rs.index}>
                    {columns.map((column) => {
                      ...
                      if(column.id !== 'action' && column.id ==='doc'){
                        ...
                        return(
                          <TableCell key={column.id} align={column.align}>
                          <ActionButton contract ={contract} lId={lId}  verified={verified} column = {column}/>
                          </TableCell>
                        )
                      }
                    })}
